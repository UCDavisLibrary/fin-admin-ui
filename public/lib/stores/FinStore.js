...
class FinStore extends BaseStore {
  ...
  setVersions(path, versions) {
    this.data.versions[path] = {id: path, payload: versions};
    this.emit(this.events.VERSIONS_UPDATE, this.data.versions[path]);
  }

  getVersions(path) {
    return this.data.versions[path];
  }
